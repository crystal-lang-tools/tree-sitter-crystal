...
EXPECTED_TO_FAIL_FILEPATH = "#{__DIR__}/crystal_parse_stdlib_expected_to_fail.txt"
...
stdlib_files.each do |file|
  ...
  tests << CorpusTest.new(test_name, [":language(crystal)"], File.read(file), expected_output, !test_name.in?(expected_fail))
end

tests.select(&.runnable?).each do |test|
  if test.run
    passed += 1
  else
    failed << test.name
